...
# ahrko_top_10_up_dn, ahrko_top_10_up_dn
...
ahrko_top_10_up_dn$`Neg log10(adj. p-val)`=-log10(ahrko_top_10_up_dn$padj)
ahrko_top_10_dn$`Neg log10(adj. p-val)`=-log10(ahrko_top_10_dn$padj)
ahrko_top_10_up$`Neg log10(adj. p-val)`=-log10(ahrko_top_10_up$padj)

ahrko_top_10_up_dn$Direction=ifelse(ahrko_top_10_up_dn$log2FoldChange > 0,"Up","Down")
ahrko_top_10_dn$Direction="Down"
ahrko_top_10_up$Direction="Up"


intersect(ahrko_top_10_up_dn$Ensembl,ahrko_top_10_up$Ensembl)
intersect(ahrko_top_10_up_dn$Ensembl,ahrko_top_10_dn$Ensembl)


plot_to_save=ggplot(ahrko_top_10_up_dn, aes(y = reorder(Symbol, log2FoldChange), x = log2FoldChange,size=`Neg log10(adj. p-val)`,color=Direction)) + 
  geom_point(data = ahrko_top_10_dn, aes(y = reorder(Symbol, log2FoldChange), x = log2FoldChange,size=`Neg log10(adj. p-val)`)) +
  geom_point(data = ahrko_top_10_up, aes(y = reorder(Symbol, log2FoldChange), x = log2FoldChange,size=`Neg log10(adj. p-val)`)) +
  scale_color_manual(values = c("Up" = "red", "Down" = "blue"))+
  xlim(-14,21) + geom_vline(xintercept = 0,linetype = "longdash") + ylab("Gene ID")+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) + ggtitle("AHRKO vs WT") + theme(plot.title = element_text(hjust = 0.5),axis.text = element_text(size=5),axis.title = element_text(size=7))+guides(colour=FALSE)+theme(plot.title = element_text(size=5))+theme(legend.title = element_text(size=5),legend.text = element_text(size=5))


tiff(file="AHRKO_vsWT_top_10_genes_logFC_plot.tiff",res=300,height = 1500,width = 1500)
grid.draw(plot_to_save)
dev.off()

write.table(ahrko_top_10_up_dn,file="Top_10_up_down_genes_by_logFC_AHRKO_vsWT.txt",col.names = T,row.names = F,sep="\t",quote = F)
